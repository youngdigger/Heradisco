document.getElementById('reserva-form').addEventListener('submit', async (event) => {
  ...
  const tipolugar = document.getElementById('tipolugar').value; // Obtén el valor del select
  ...
  try {
    const response = await fetch('/reservar', {
      ...
      body: JSON.stringify({ nombre, email, fecha, personas, tipolugar }), // Incluye tipolugar en el body
    });
